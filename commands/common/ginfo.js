...
{ constructor()
  ...
  {	let guildo = await fetchMs(message);

    let members = guildo.members;

    let roleNames = message.guild.roles.map(r => r.toString()).slice(0, 20).join(', ');
	if (message.guild.roles.map(r => r.toString()).length > 20) roleNames = roleNames + '...';
